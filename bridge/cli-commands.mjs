 ...
 * Used by server (validation) and dashboard (UI rendering).
 */
...
/**
 * Convert an action object { command, args: {name: val}, options: {name: val} }
 * to [command, [...positionalArgs, ...flags]] suitable for session.sendCommand().
 */
export function actionToCommandArgs(action) {
  const def = CLI_COMMANDS[action.command];
  if (!def) throw new Error(`Unknown command: ${action.command}`);

  const positional = (def.args || [])
    .map(a => action.args?.[a.name])
    .filter(v => v != null && v !== '');

  const flags = [];
  for (const [k, v] of Object.entries(action.options || {})) {
    if (v === true) flags.push(`--${k}`);
    else if (v !== false && v != null && v !== '') flags.push(`--${k}`, String(v));
  }

  return { command: action.command, cmdArgs: [...positional, ...flags] };
}
