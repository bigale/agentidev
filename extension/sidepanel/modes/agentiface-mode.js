...
function handleGenerate() {
  ...
  chrome.runtime.sendMessage({
    ...
    if (response?.success) {
      ...
      // Open wrapper tab to show the generated UI
      const extId = chrome.runtime.id;
      chrome.tabs.create({
        url: `chrome-extension://${extId}/smartclient-app/wrapper.html?mode=playground`,
        active: false,
      });
      appendLog('Opened in new tab', 'success');
      showError(null);
    } else {
...
async function handleAgentGenerate(prompt) {
  ...
  try {
    ...
    // Auto-publish as plugin so it appears in the plugin list
    try {
      const name = prompt.length > 40 ? prompt.substring(0, 40) + '...' : prompt;
      chrome.runtime.sendMessage({
        type: 'SC_PUBLISH_PLUGIN',
        name: name,
        description: 'Generated by agent: ' + prompt,
        config: null, // uses playgroundSession.config
      }, (resp) => {
        if (resp?.success) {
          appendLog('Saved as plugin: ' + (resp.id || name), 'success');
        }
      });
    } catch {} // non-fatal
