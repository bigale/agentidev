...
The zato-alt.md analyzed mapping Zato's 12 capabilities to browser libraries. But there's a simpler path: **run actual Zato in CheerpX**.

CheerpX runs x86 Linux. Zato is Python. With CheerpX's Python support (PYTHONHASHSEED=0 workaround), a minimal Zato instance could potentially run inside the browser:
- Zato services as Python classes
- SQLite for persistence (already works in CheerpX)
- REST channels via CheerpX networking (limited but possible)

This is the "real thing in the browser" approach vs. the "reimplemented using browser primitives" approach. Both are valid. The bridge server IS the reimplementation; CheerpX-Zato IS the original.

BrowserPod's roadmap includes Python support. When that lands, Zato-in-BrowserPod with Portals for REST channels becomes viable without the CheerpX entropy workaround.
